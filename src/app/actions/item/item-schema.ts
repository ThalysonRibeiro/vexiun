...
export const itemFormSchema = z.object({
  ...
  assignedTo: z.string().cuid(ERROR_MESSAGES.VALIDATION.INVALID_ID).nullable().optional()
});
export const createItemFormSchema = groupIdFormSchema.merge(itemFormSchema);

export const updateItemFormSchema = itemIdFormSchema.merge(
...
export const changeStatusFormSchema = itemIdFormSchema.extend({
  workspaceId: z
    ...
    .min(1, ERROR_MESSAGES.VALIDATION.REQUIRED_FIELD)
    .cuid(ERROR_MESSAGES.VALIDATION.INVALID_ID),
  newStatus: z.enum([EntityStatus.ACTIVE, EntityStatus.ARCHIVED, EntityStatus.DELETED])
});
